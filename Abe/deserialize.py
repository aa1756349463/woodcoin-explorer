...
def deserialize_Transaction(d, transaction_index=None, owner_keys=None, print_raw_tx=False):
    ...
    if print_raw_tx == True:
        result += "Transaction hex value: " + codecs.encode(d['__data__'], 'hex') + "\n"
    return result
...
def deserialize_MerkleTx(d, transaction_index=None, owner_keys=None):
    ...
    result = "block: " + codecs.encode(d['hashBlock'][::-1], 'hex')
...
def script_GetOp(the_input):
    i = 0
    while i < len(the_input):
        vch = None
        print('bytes[i]: {0}'.format(the_input[i]))
        opcode = ord(bytes([the_input[i]]))
        i += 1

        if opcode <= opcodes.OP_PUSHDATA4:
            nSize = opcode
            if opcode == opcodes.OP_PUSHDATA1:
                if i + 1 > len(the_input):
                    vch = "_INVALID_NULL"
                    i = len(the_input)
                else:
                    nSize = ord(the_input[i])
                    i += 1
            elif opcode == opcodes.OP_PUSHDATA2:
                if i + 2 > len(the_input):
                    vch = "_INVALID_NULL"
                    i = len(the_input)
                else:
                    (nSize,) = struct.unpack_from('<H', the_input, i)
                    i += 2
            elif opcode == opcodes.OP_PUSHDATA4:
                if i + 4 > len(the_input):
                    vch = "_INVALID_NULL"
                    i = len(the_input)
                else:
                    (nSize,) = struct.unpack_from('<I', the_input, i)
                    i += 4
            if i+nSize > len(the_input):
                vch = "_INVALID_" + str(the_input[i:])
                i = len(the_input)
            else:
                vch = the_input[i:i+nSize]
                i += nSize
...
def decode_script(bytes_input):
    result = ''
    for (opcode, vch) in script_GetOp(bytes_input):
        print('opcode: {0}'.format(opcode))
        print('vch: {0}'.format(vch))
        if len(result) > 0:
            result += " "
        if opcode <= opcodes.OP_PUSHDATA4:
            result += "%d:"%(opcode,)
            shortened_hex = short_hex(vch)
            if type(shortened_hex) == bytes:
                result += shortened_hex.decode()
            else:
                result += shortened_hex
        else:
